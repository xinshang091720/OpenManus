...
Resolve each DWG floor from an explicit user mapping first, then the drawing title/frame, then a filename hint. A filename is never a filtering or naming rule. A drawing titled as a floor range applies to every floor in that range. If the floor is genuinely ambiguous, return `selection_required` before any Revit create, name, or SaveAs operation. Ask once for the exact DWG-to-floor mapping and continue with `floor_overrides`, using the returned absolute `dwg_path` and confirmed `floor_numbers`. Do not rename, copy, or omit source DWGs to force a result.
