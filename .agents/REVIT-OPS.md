...
- 只有用户**明确**要求"赋值/重新赋值/补充/重建"时才调用 `revit_run_ifc_assignment`；"质检"绝不触发赋值。当用户多步骤任务已明确要求做 IFC 赋值且模型名专业明确（如含 AR）时，直接自主执行赋值，严禁调用 `ask_human` 重复确认。
- 房间成功返回 `saved_model_path` 后，后续导出直接用该结果模型，不要重新打开源 RVT（会丢房间）。
- 专业代码：`AR/A`=建筑、`ST/S/FS/SS`=结构、`AC/M`=通风空调、`PD/P`=给排水、`EL/E/T`=电气；`G` 有歧义（总图/燃气）必须问。其它专业明确时不问。
