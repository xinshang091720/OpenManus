...
class WebSearch(BaseTool):
    ...
    async def _try_all_engines(
        ...
        for engine_name in engine_order:
            ...
            # Some Chinese search packages return internal relative paths such
            # as `/s?wd=...`. They are not usable by a browser or fetcher, and
            # must not prevent a later engine from being tried.
            valid_items = [
                item for item in search_items if self._is_absolute_http_url(item.url)
            ]
            if not valid_items:
                failed_engines.append(f"{engine_name} (no valid URLs)")
                continue
