    ...
    async def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if getattr(result, "isError", False):
                return ToolResult(
                    error=content_str or f"MCP tool '{self.original_name}' failed"
                )
...
class MCPClients(ToolCollection):
    ...
    def __init__(self):
        ...
        # Runtime runs create separate Manus instances.  MCP connections must
        # be owned by their instance rather than shared across conversations.
        self.sessions = {}
        self.exit_stacks = {}
    ...
    async def connect_sse(
        ...
        await self._initialize_and_list_tools(
            server_id, allowed_tools=allowed_tools, expose_original_names=expose_original_names
        )
    ...
    async def connect_stdio(
        ...
        # The MCP SDK intentionally starts stdio children with a safe, reduced
        # environment.  Runtime-owned children therefore need an explicit,
        # narrow allowlist for the configuration they actually consume.
        ...
        *,
        ...
        for tool in response.tools:
            ...
            if allowed_tools is not None and original_name not in allowed_tools:
                continue
            tool_name = (
                self._sanitize_tool_name(original_name)
                if expose_original_names
                else self._sanitize_tool_name(f"mcp_{server_id}_{original_name}")
            )
