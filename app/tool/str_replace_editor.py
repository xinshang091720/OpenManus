"""Local Windows file and directory manipulation tool."""
...
class StrReplaceEditor(BaseTool):
    """A tool for viewing, creating, and editing files on the local host."""
    ...
    def _get_operator(self) -> FileOperator:
        """Desktop Runtime file operations always use the local filesystem."""
    ...
    async def _view_directory(path: PathLike, operator: FileOperator) -> CLIResult:
        ...
        # Avoid the Unix-only `find` invocation when using a local Windows operator.
            ...
            for current_root, directories, files in os.walk(root):
                ...
                for name in [*directories, *files]:
                    if not name.startswith("."):
                        entries.append(str(current / name))
